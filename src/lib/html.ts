...
export function createElementsFromHTML(htmlString: string): Record<string, HTMLElement> {
	...
	const elements = Array.from(doc.body.querySelectorAll('[data-key]').values()) as HTMLElement[];
	const obj = Object.fromEntries(
		elements.map((element) => [element.getAttribute('data-key')!, element])
	);
	elements.forEach((element) => element.removeAttribute('data-key'));
	return obj;
}
